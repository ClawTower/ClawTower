/*
 * Test program for libclawguard.so v2
 *
 * Run: LD_PRELOAD=./libclawguard.so ./test_interpose
 *
...
static uint16_t (*clawguard_get_threat_level)(void);
static uint16_t (*clawguard_get_alert_state)(void);
static uint32_t (*clawguard_get_ring_count)(void);
static uint32_t (*clawguard_get_ring_head)(void);
static void     (*clawguard_reset)(void);
static syscall_event_t *(*clawguard_get_ring_event)(uint32_t idx);
...
static void test_ring_buffer(void) {
    ...
    uint32_t before = clawguard_get_ring_count();

    /* Open a regular file — should record to ring buffer */
    int fd = open("/dev/null", O_RDONLY);
    if (fd >= 0) close(fd);

    uint32_t after = clawguard_get_ring_count();
    ...
    CHECK(clawguard_get_ring_count() >= 10, "expected >=10 events");
}
...
static void test_threat_scoring(void) {
    ...
    uint16_t before = clawguard_get_threat_level();
    ...
    uint16_t after = clawguard_get_threat_level();
    ...
    uint16_t state = clawguard_get_alert_state();
    if (after < 300) {
...
static void test_sensitive_fd_tracking(void) {
    ...
    if (fd >= 0) {
        uint16_t before_read = clawguard_get_threat_level();
        char buf[32];
        ssize_t n = read(fd, buf, sizeof(buf));
        uint16_t after_read = clawguard_get_threat_level();
        ...
        uint16_t before_null = clawguard_get_threat_level();
        fd = open("/dev/null", O_RDONLY);
        if (fd >= 0) {
        ...
        CHECK(clawguard_get_threat_level() <= before_null + 1,
...
static void test_ring_wrap(void) {
    ...
    CHECK(clawguard_get_ring_count() >= 300, "expected >=300 total events");

    /* Head should have wrapped */
    uint32_t head = clawguard_get_ring_head();
    TEST("head pointer wraps correctly");
    CHECK(head >= 300, "head should be >= 300 (raw, mod 256 for index)");

    /* Events should still be readable */
    syscall_event_t *evt = clawguard_get_ring_event(head % 256);
...
static void test_connect_scoring(void) {
    ...
    uint16_t before_connect = clawguard_get_threat_level();
    ...
    if (sock >= 0) {
        ...
        uint16_t after_connect = clawguard_get_threat_level();
...
static void test_introspection(void) {
    ...
    CHECK(clawguard_get_threat_level() == 0, "threat not 0 after reset");

    TEST("reset clears ring count");
    CHECK(clawguard_get_ring_count() == 0, "ring count not 0 after reset");

    TEST("reset clears alert state");
    CHECK(clawguard_get_alert_state() == THREAT_NORMAL, "alert not NORMAL after reset");

    /* Get event from empty ring */
    syscall_event_t *evt = clawguard_get_ring_event(0);
    TEST("ring event at index 0 accessible");
    CHECK(evt != NULL, "event pointer should not be NULL");

    TEST("out of bounds index returns NULL");
    CHECK(clawguard_get_ring_event(999) == NULL, "expected NULL for OOB index");
}
...
static void test_decay(void) {
    ...
    uint16_t initial = clawguard_get_threat_level();
    ...
    if (initial > 0) {
        ...
        uint16_t after_decay = clawguard_get_threat_level();
...
int main(void) {
    printf("╔══════════════════════════════════════════════════╗\n");
    printf("║   libclawguard.so v2 — Test Suite               ║\n");
    printf("╚══════════════════════════════════════════════════╝\n");

    /* Resolve introspection API from LD_PRELOAD'd library */
    clawguard_get_threat_level = dlsym(RTLD_DEFAULT, "clawguard_get_threat_level");
    clawguard_get_alert_state  = dlsym(RTLD_DEFAULT, "clawguard_get_alert_state");
    clawguard_get_ring_count   = dlsym(RTLD_DEFAULT, "clawguard_get_ring_count");
    clawguard_get_ring_head    = dlsym(RTLD_DEFAULT, "clawguard_get_ring_head");
    clawguard_get_ring_event   = dlsym(RTLD_DEFAULT, "clawguard_get_ring_event");
    clawguard_reset            = dlsym(RTLD_DEFAULT, "clawguard_reset");

    if (!clawguard_get_threat_level || !clawguard_reset) {
        printf("\n⚠️  Not running with LD_PRELOAD! Run with:\n");
        printf("   LD_PRELOAD=./libclawguard.so ./test_interpose\n\n");
        return 1;
    }
